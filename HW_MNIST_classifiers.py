...
# 
# Use Pipeline. I do not feel that we should standardize all the 784 pixels, but 
...
classifiers = {
    ...
    "DecisionTreeClassifier (gini)": DecisionTreeClassifier(criterion='gini', max_depth=10),
    "DecisionTreeClassifier (entropy)": DecisionTreeClassifier(criterion='entropy', max_depth=10),
...
for name, model in classifiers.items():
    ...
    runtime = time.time() - start_time
    print(f"Model: {name}, CV Mean Accuracy: {cv_scores.mean():.4f}, Runtime: {runtime:.2f}s")
